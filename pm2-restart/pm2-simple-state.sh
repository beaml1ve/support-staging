...
save_current_state() {
    ...
    STATE_FILE="/var/www/beamdevlive/.pm2/custom_state_${TIMESTAMP}.json"
    ...
    echo "Standard PM2 dump saved to: /var/www/beamdevlive/.pm2/dump.pm2"
    
    # Create a symlink to the latest state
    ln -sf "$STATE_FILE" "/var/www/beamdevlive/.pm2/latest_custom_state.json"
    
    echo "Latest state linked at: /var/www/beamdevlive/.pm2/latest_custom_state.json"
}

# Function to restore state
restore_state() {
    local state_file="${1:-/var/www/beamdevlive/.pm2/latest_custom_state.json}"
    
    if [ ! -f "$state_file" ]; then
        echo "Error: State file not found: $state_file"
        echo "Available state files:"
        ls -la /var/www/beamdevlive/.pm2/custom_state_*.json 2>/dev/null || echo "No custom state files found"
        exit 1
    fi
...
case "$1" in
    ...
    "show"|"status")
        ...
        ;;
    *)
        echo "Usage: $0 {save|restore [state_file]|show}"
        echo ""
        echo "Commands:"
